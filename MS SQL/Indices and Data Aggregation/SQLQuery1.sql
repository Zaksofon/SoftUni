...
--9. Age Groups
SELECT [AgeGroup], COUNT([Id]) AS [WizardCount]  FROM
	(
  SELECT *,
	CASE 
...
SELECT * 
INTO [EmployeesWith30000Salary]
FROM [Employees]
WHERE [Salary] > 30000

DELETE FROM [EmployeesWith30000Salary]
WHERE [ManagerID] = 42

UPDATE [EmployeesWith30000Salary]
SET [Salary] += 5000
WHERE [DepartmentID] = 1

SELECT [DepartmentID], AVG([Salary]) AS [AverageSalary] FROM [EmployeesWith30000Salary]
GROUP BY [DepartmentID]
...
WHERE [Salary] >
			(
		   SELECT AVG([Salary]) FROM [Employees] AS esub 
